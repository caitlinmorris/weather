...
| **Codex CLI** | Full JSONL transcripts at `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl` — includes assistant text (richer than Warp). **Now the ACTIVE target** (Lara switched from Warp, 2026-07-16); structure probe sent (docs/codex-probes.sh). CaptureSource prerequisite already built | **~0.5 day** once probe returns | by-date layout: allowlist requires header-line peek (metadata-only; disclosed in consent walkthrough) |
