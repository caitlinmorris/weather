...
def test_boards_multi(monkeypatch):
    config = _reload_config(
        ...
        RELAY_URL_VEE="https://b.example", RELAY_TOKEN_VEE="t2",
        PRESENCE_TIER_VEE="presence",
    )
    bs = config.boards()
    assert [b["name"] for b in bs] == ["dan", "vee"]
...
def test_composite_merge_tags_boards_and_dedups(tmp_path, monkeypatch):
    ...
    (tmp_path / "group_cache_vee.json").write_text(json.dumps(
        {"per_person": [{"person_id": "caitlin", "states": [dict(shared)]},
                        {"person_id": "vee", "states": [dict(shared)]}]}))
    data = build_page.group_cache_data()
    people = {p["person"]: p for p in data}
    assert set(people) == {"caitlin", "dan", "vee"}
    ...
    assert people["vee"]["states"][0]["board"] == "vee"
